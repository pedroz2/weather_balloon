...
std_acc = std(acceleration);

% Max acceleration at 1.0017 g at 13058 m
figure()
hold on
line([0.82 1.02],[13852 13852],'LineWidth',1)
...
legend("\fontsize{12}Peak Accleration at 13852 m");
title("\fontsize{16}Magnitude of Acceleration vs Altitude");
...
ax1 = subplot(1,2,1);
subplot(1,2,1);
plot(difference_smooth(500:8000), altitude(500:8000), '-r', 'LineWidth',1);
xlim([-.25 .25]);
title("\fontsize{16}During Ascent");
...
xlim([-.05 .45]);
title("\fontsize{16}During Descent")
ylabel("\fontsize{14}Altitude (m)");
xlabel("\fontsize{14}Acceleration Difference (g)");
hold off
